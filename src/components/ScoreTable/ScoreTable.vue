  ...
  <div>
            ...
            <el-card>
              ...
              >
                ...
                <el-button
                  ...
                  type="text"
                  size="small"
                  @click="handleCreate(props.$index)"
                >
                  新建扣分
                </el-button>
              </div>
    ...
    >
      <el-form :model="form">
        ...
        >
          <el-input-number
            :value="form.region!==''?tableData[selected].standards[form.region].score:0"
            :disabled="true"
          />
        </el-form-item>
        <el-form-item
        ...
        >
          ...
          >
            <el-option
              v-for="standard in tableData[selected].standards"
              :key="standard.index"
    ...
    <div v-if="1-auth">
      <el-select
        v-model=" select_index1"
        clearable
        placeholder="选择板块"
      >
        <el-option
          v-for="item in first_chart"
          :key="item.index"
          :label="item.name"
          :value="item.index"
        />
      ...
      <el-select
        v-model=" select_index2"
        clearable
        placeholder="选择评分指标"
        :disabled="select_index1===''"
      >
        <el-option
          v-for="item in second_chart[select_index1]"
          :key="item.index"
          :label="item.name"
          :value="item.index"
        />
  ...
  methods: {
    ...
    drawChart3() {
      ...
      let option3 = {
        ...
        xAxis: {
          data: this.second_chart[this.select_index1][
            this.select_index2
          ].issues.map(data => {
            return data.reason;
          })
    ...
    arraySpanMethod({ row, rowIndex, columnIndex }) {
      ...
      if (columnIndex <= this.auth + 2 && columnIndex > 0) {
        if (
          ...
          row.serial === this.tableData[rowIndex + 1].serial
        ) {
          return [2, 1];
        }
        if (
    ...
    calChart() {
      ...
      for (let i = 1; i <= this.tableData.length; i++) {
        if (
          ...
          this.tableData[i].serial === "title"
        ) {
          this.first_chart.push(
            Object.assign({}, temp, { name, index: indexTitle })
          );
    ...
    handleEdit(index, row) {
      ...
      this.form.region = parent.standards.filter(data => {
        if (data.reason === row.reason) return data;
      })[0].index;
      this.editing = true;
      this.dialogFormVisible = true;
    },
    ...
    computedScore(index) {
      // 计算当前评分指标的得分
      return this.tableData[index].issues.reduce((sum, data) => {
        return sum + data.minusScore;
      }, 0);
    },
    changeSelection() {
      // 切换评分标准时的回调
      if (this.form.region !== "") {
        this.form.num = this.tableData[this.selected].standards.filter(data => {
          if (data.index === this.form.region) return data;
        })[0].score;
      }
    },
    confirmCreate() {
      ...
      if (this.editing) {
        let reason = parent.standards.filter(data => {
          if (data.index === this.form.region) return data;
        })[0].reason;
        parent.issues.filter(data => {
          if (data.reason === reason) return data;
        })[0].minusScore = this.form.num;
        parent.minusScore = this.computedScore(this.selected);
        this.editing = false;
      } else {
        let issue = {};
        issue.minusScore = this.form.num;
        issue.reason = parent.standards.filter(data => {
          if (data.index === this.form.region) return data;
        })[0].reason;
        issue.father = this.selected;
        let flag = parent.issues.filter(data => {
          if (data.reason === issue.reason) return data;
        }).length;
        if (flag) {
          alert("已存在同名扣分项");
